...
app.post('/api/calculate', async (req, res) => {
    try {
        const { loanAmount, amortization, terms, marginAbovePrime, startDate } = req.body;
        ...
        if (!startDate) throw new CustomError(`Start date must be set, received: ${startDate}`, 400);

        const schedule = await calculateAmortizationSchedule({ loanAmount, amortization, terms, marginAbovePrime, startDate })
